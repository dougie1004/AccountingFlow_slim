...
pub fn get_headers(bytes: &[u8], file_name: &str) -> Result<Vec<String>, SystemError> {
    let ext = std::path::Path::new(file_name).extension().and_then(|s| s.to_str()).unwrap_or_default().to_lowercase();
    if ext == "xlsx" || ext == "xls" {
        let mut excel: Xlsx<Cursor<&[u8]>> = calamine::open_workbook_from_rs(Cursor::new(bytes)).map_err(|e: XlsxError| SystemError::InvalidFormat(format!("Excel Read Error: {}", e)))?;
        let sheet = excel.sheet_names().get(0).ok_or(SystemError::InvalidFormat("No sheets".to_string()))?.clone();
        let range = excel.worksheet_range(&sheet).map_err(|e: XlsxError| SystemError::InvalidFormat(format!("Sheet Range Error: {}", e)))?;
        if let Some(row) = range.rows().next() {
            return Ok(row.iter().map(|c| c.to_string()).collect());
        }
    } else {
        let decoded = crate::ai::robust_parser::detect_and_decode(bytes).map_err(SystemError::from)?;
        let mut rdr = ReaderBuilder::new().has_headers(false).from_reader(decoded.as_bytes());
        if let Some(result) = rdr.records().next() {
            let record = result.map_err(|e| SystemError::InvalidFormat(format!("CSV Read Error: {}", e)))?;
            return Ok(record.iter().map(|s| s.to_string()).collect());
        }
    }
    Err(SystemError::EmptyFile)
}
...
pub fn process_with_mapping(
    ...
    if !col_map.contains_key("tx_date") || !col_map.contains_key("amount") {
        return Err(SystemError::InvalidFormat("Missing required mappings (Date, Amount)".into()));
    }
    ...
    for result in rdr.records() {
        ...
        let mut tx = ParsedTransaction {
            ...
            source_type: Some(TransactionSource::BankFile), // Assuming bank format if user mapped columns
