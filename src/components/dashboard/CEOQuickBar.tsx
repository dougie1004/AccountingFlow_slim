...
export const CEOQuickBar: React.FC<CEOQuickBarProps> = ({ financials, avgMonthlyBurn }) => {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-in fade-in slide-in-from-top-4 duration-700">
            ...
            <div className={`relative group overflow-hidden bg-[#151D2E] border border-white/5 p-8 rounded-[2.5rem] shadow-2xl transition-all duration-500`}>
            ...
            <div className="bg-[#151D2E] border border-white/5 p-8 rounded-[2.5rem] relative overflow-hidden group shadow-xl">
                ...
                <div className="flex items-center justify-between mb-2">
                    ...
                    <InfoTooltip
                        ...
                        content="전체 유동 자산 중 고정 지출 및 확정 부채를 제외한 실질 운용 가능 자본입니다."
                        contextualTip={`현재 유동성 구조 분석 결과, ₩${(operationalFreeCash / 10000).toFixed(0)}만원 수준의 유동성 확보를 권장합니다.`}
                    />
                </div>
            ...
            <div className={`p-8 rounded-[2.5rem] border transition-all relative overflow-hidden group shadow-xl ${taxReserve > 0 ? 'bg-amber-500/5 border-amber-500/20' : 'bg-[#151D2E] border-white/5'
                }`}>
                <div className="flex items-center justify-between mb-2">
                ...
                <h4 className="text-slate-400 font-bold text-sm tracking-tight mb-4">Accrued Tax Liabilities</h4>
                <div className="flex flex-col gap-1">
