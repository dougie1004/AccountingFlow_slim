...
#### Rule 7: 진실 모드 원칙 (Truth over Narrative)
- 엔진은 특정 목표(Target)나 사업계획서의 숫자를 맞추기 위해 로직을 역산(Calibration)하거나 인위적으로 보정(Tuning)해서는 안 된다.
- 엔진의 유일한 목적은 "진실된 시뮬레이션(Simulation Mode)"이며, 목표 달성 여부와 상관없이 설계된 로직에 따른 정직한 결과만을 도출한다.
