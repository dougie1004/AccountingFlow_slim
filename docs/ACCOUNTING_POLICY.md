...
#### Rule 11: 절대적 텍스트 주권 (Absolute Text Sovereignty - Encoding)
- 대한민국 금융 데이터의 특수성(EUC-KR/CP949)과 현대적 표준(UTF-8) 사이의 인코딩 충돌로 인한 데이터 파손(Mojibake)은 '데이터 무결성 파괴'로 간주한다.
- 모든 파일 입출력 로직은 사용자의 개입 없이 인코딩을 자동으로 감지(Auto-detect)하고 변환해야 하며, 어떤 환경에서도 글자 깨짐이 발생해서는 안 된다.
- "글자가 깨진 데이터는 존재하지 않는 데이터보다 위험하다"는 원칙을 고수한다.
